...
export function buildFormDefaults({
  ...
  const iconsExport =
    icons === "none"
      ? false
      : `export { icons } from "${iconSetPackage(icons).name}";`;
  ...
  return join2(
    ...
    join(
      `export { theme } from "${effectiveThemePackage.name}";`,
      ...extraWidgetImports
    ),
    iconsExport,
    ...
    !(validatorName in VALIDATOR_ADAPTERS) &&
      `export { createFormValidator as validator } from "${externalValidatorPackage(validatorName as ExternalValidator).name}";`
  );
}
...
const VALIDATOR_ADAPTERS: Record<
  BuilderValidatorName & SchemaValidator,
  string
> = {
...
export function buildFormDotSvelte({
  ...
  const validatorAdapter = isSchemaValidator(validatorName)
    ? VALIDATOR_ADAPTERS[validatorName]
    : false;
  const code = join(
    `<script lang="ts">
  ...
  ${SCHEMA_FACTORIES[validatorName](schema)};
...
function* themeOptionalDeps<T extends Theme>(
  theme: T,
  widgets: Iterable<ExtraWidgetFileNames[T]>
) {
  for (const w of widgets) {
...
export function buildInstallSh<T extends PlaygroundTheme>({
  ...
  const deps = [
    ...filterPackageDependencies(
      externalValidatorPackage(validatorName as ExternalValidator).dependencies,
      false
    ),
    ...filterPackageDependencies(
      ...
      Array.from(
        themeOptionalDeps(
          t,
          widgetFileNames as Set<ExtraWidgetFileNames[typeof t]>
        )
      )
    ),
  ];

  let cmd = `npm i ${formPackage.name} ${themePkg.name} ${externalValidatorPackage(validatorName as ExternalValidator).name}`;
  if (icons !== "none") {
