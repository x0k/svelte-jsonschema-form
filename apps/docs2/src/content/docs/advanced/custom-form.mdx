...
import customFormComponentCode from '@/components/custom-form.svelte?raw';

## Factory

Even with a simple setup, resulting code is very verbose.
Therefore, after choosing a theme, validator and translation, it is convenient to create a wrapper around the `createForm3`.
...
- If you are sure that you will always call the `createForm3` function with passing the object literal, you can use the following code instead of `Proxy`:
