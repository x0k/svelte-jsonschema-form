...
import clientCode from "examples/sveltekit-starter/src/routes/form-actions/+page.svelte?raw";
import serverCode from "examples/sveltekit-starter/src/routes/form-actions/+page.server?raw";
import clientFlexCode from "examples/sveltekit-starter/src/routes/form-actions-flex/+page.svelte?raw";
import serverFlexCode from "examples/sveltekit-starter/src/routes/form-actions-flex/+page.server?raw";
...
For correct operation, you must use ID Builder from the `@sjsf/sveltekit`.
...
export const actions = {
  default: createAction({/* options */}, (data, event, meta) => {
    /* logic */
  }),
} satisfies Actions;
```
...
Compare the current value snapshot with the initial value to detect unsaved changes.
...
- `onInvalid` — triggered when client-side validation errors are present
- `onValidationProcessError` — triggered when an error occurs during validation
