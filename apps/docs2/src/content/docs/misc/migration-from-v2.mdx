...
### New `createIdBuilder`, `createMerger` and `createValidator` form options

New options replace the `validator`, `merger`, `idSeparator` and `idPseudoSeparator` properties.
...
The context now provides the entire `form` object, while `form.value` is a public value accessor.
...
- Removed the `idSeparator` and `idPseudoSeparator` form parameters
...
To preserve the previous behavior, replace:
...
- `svelte@^5.39.0`
- `@sveltejs/kit@^2.42.0` (`@sjsf/sveltekit`)
