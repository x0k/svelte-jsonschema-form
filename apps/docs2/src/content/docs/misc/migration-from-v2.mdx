...
<Npm pkg="json-schema-merge-allof" />
...
The SvelteKit integration is now provided by the `@sjsf/sveltekit3` package,
which requires [SvelteKit 3](https://svelte.dev/docs/kit/migrating-to-sveltekit-3).
For SvelteKit 2 keep using the legacy `@sjsf/sveltekit` package.

Now, for the integration to work correctly,
you must use ID Builder from the `@sjsf/sveltekit3` module.
...
See the [Form actions](/integrations/form-actions/#advanced-api) page for more information.
...
- `@sveltejs/kit@^3.0.0-next.25` (`@sjsf/sveltekit3`)
