...
export interface FormState<T> {
  ...
  readonly [FORM_ERRORS]: SvelteMap<FieldPath, string[]>;
...
`getFieldsValidationMode`, `validateFormValue`, `validateFormValueAsync`,
`getValueSnapshot`

Use the standalone `validate(form, handlers)` and `reset(form)` functions
(with the `handlers(form)` form attachment) to validate or reset the whole form.
...
<LinkCard
  title="Reactive flow"
  href={withBase("/form/reactive-flow/")}
  description="See the diagrams for how queries, commands, and event flows connect to FormState."
/>
