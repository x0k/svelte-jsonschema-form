...
For example, we can use `root` as an argument for `getComponent` because
`root` is `FoundationalComponentType`:

```svelte
<script lang="ts">
  ...
  const Root = $derived(getComponent(ctx, "root", config));
</script>

<Root {config} {children} />
```
...
<LinkCard
  ...
  description="Create your own form components"
  href={withBase("/guides/custom-components/")}
/>
<LinkCard
  title="Component casting"
  description="Adapt interchangeable components for use in your forms"
