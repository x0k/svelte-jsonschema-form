...
type ResolveFieldType = (config: Config) => UnifiedFieldType
...
Let's talk about `UnifiedFieldType`.

## Foundational components

The `UnifiedFieldType` type is a subset of fields from `FoundationalFieldType`
that have the same signature (except for the `value` property).
