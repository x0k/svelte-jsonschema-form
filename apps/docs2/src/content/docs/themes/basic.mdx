...
export const pkg = themePackage('basic')
...
<Tabs>
  <TabItem label='Form'>
    <FormCard style={PLAYGROUND_SJSF_THEME_STYLES['basic']}>
      <Form client:only="svelte" />
    </FormCard>
  </TabItem>
  <TabItem label='Form (Pico CSS)'>
    <FormCard style={PLAYGROUND_SJSF_THEME_STYLES['pico']}>
      <Form client:only="svelte" />
    </FormCard>
  </TabItem>
