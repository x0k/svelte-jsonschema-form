...
import basicResolverCode from "pkgs/form/src/form/resolvers/basic?raw";
import compatResolverCode from "pkgs/form/src/form/resolvers/compat?raw";
...
For this schema, the `basic` resolver will select `stringField`,
and `stringField` will display the `textWidget` widget by default
although you probably wanted to see select or radio widgets.

To achieve this you need to specify the field in the UI schema:
...
when a schema with `enum` is detected:
...
Some interchangeable components (`tagsField`, `filesField`, etc.) cannot be assigned directly
in the UI schema due to type incompatibilities.

<LinkCard
  title="Component casting"
  description="Handle interchangeable components that cannot be used directly"
  href={withBase("/guides/component-casting/")}
/>
