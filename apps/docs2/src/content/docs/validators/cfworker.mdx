---
title: cfworker/json-schema
type: package
packageCodeName: "cfworker"
sidebar:
  order: 3
---

import { LinkButton } from "@astrojs/starlight/components";
import { externalValidatorPackage } from "meta";

import { getPackageDependencies } from "@/shared";
import Npm from "@/components/npm.astro";
import Deps from "@/components/peer-dependencies.astro";
import Demo from "@/components/demo.svelte";

export const pkg = externalValidatorPackage("cfworker");

Form validator implementation based on [@cfworker/json-schema](https://github.com/cfworker/cfworker/tree/main/packages/json-schema).

## Installation

<Npm pkg={getPackageDependencies(pkg)} />

<Deps pkg={pkg} />

## Example

<Demo name="cfworker" iconsPicker client:visible />

## Format support

By default, `data-url` and `color` formats are not installed. Call `setupFormFormats()` once at app startup to enable them:

```typescript
import { format } from "@cfworker/json-schema";
import { setupFormFormats } from "@sjsf/cfworker-validator";

setupFormFormats(format);
```

Custom formats can be added directly to `@cfworker/json-schema`'s global format registry:

```typescript
import { format } from "@cfworker/json-schema";

format.phone = (value) => /^\+\d{10,15}$/.test(value);
```

## Caveats

- By itself this validator does not support undefined values. But we use the
  `valueToJSON` function with the following default implementation to fix it,
  consider this overhead.

  ```typescript
  const valueToJSON = (v: FormValue) =>
    v === undefined || v === null
      ? null
      : typeof v === "object"
        ? JSON.parse(JSON.stringify(v))
        : v;
  ```

  It will also lead to incorrect error text (e.g.
  `Instance type "null" is invalid. Expected "string"`) where you would like to
  see the text `this field is required`.
  This can be fixed using [error transformation](/form/validator/).

## Async validation

This validator does not support async validation.
