<script lang="ts" module>
  ...
  declare module "@sjsf/form" {
    ...
    interface UiOptions {
      ...
      shadcn4DateRangeFormatter?: (
        range: Range<DateValue | undefined>
      ) => string;
    }
  }
...
<script lang="ts">
  ...
  const { Popover, PopoverTrigger, PopoverContent, RangeCalendar } = $derived(
    themeCtx.components
  );
  ...
  const formatDateRange = $derived.by(() => {
    const formatter = retrieveUiOption(
      ctx,
      config,
      "shadcn4DateRangeFormatter"
    );
    ...
    const placeholder = retrieveUiOption(
      ctx,
      config,
      "shadcn4DateRangePickerPlaceholder"
    );
  ...
  const parsed: Range<DateValue | undefined> = $derived({
    start:
      typeof value?.start === "string" ? parseDate(value.start) : undefined,
