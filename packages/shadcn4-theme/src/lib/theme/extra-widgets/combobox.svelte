...
<script lang="ts">
  ...
  const attributes = $derived(
    inputAttributes(ctx, config, "shadcn4ComboboxInput", handlers, {})
  );

  const triggerContent = $derived(
    labels.get(mapped.current) ?? attributes.placeholder
  );

  const emptyText = $derived(
    retrieveUiOption(ctx, config, "shadcn4ComboboxEmptyText")
  );
          ...
          {#each options as option (option.id)}
            ...
            >
              <Check
                class={cn(
                  "mr-2 size-4",
                  mapped.current !== ov && "text-transparent"
                )}
              />
              {option.label}
            </CommandItem>
