...
export function useSvelteKitMutation<T, E>(options: SvelteKitOptions<T, ActionResult, E>) {
	const jsonChunkSize = $derived(options.jsonChunkSize ?? 500000);
	const shouldReset = $derived(options.reset ?? true);
	const shouldInvalidateAll = $derived(options.invalidateAll ?? true);
	...
	return useMutation(options as MutationOptions<[T, SubmitEvent], ActionResult, E>);
}
