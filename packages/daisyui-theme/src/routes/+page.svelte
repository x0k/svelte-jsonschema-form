<script lang="ts">
	import Ajv from 'ajv';
	import { Form, ValidatorErrorType, type ValidatorError } from '@sjsf/form';
	...
	const initialSampleName = selectSample('Simple', true);
...
<div class="py-4 px-8 min-h-screen">
	...
	<div class="flex gap-2 flex-wrap pb-6 text-black">
		{#each Object.entries(samples) as [name, sample]}
			<button
				...
				class:bg-red-300={sample.status === 'broken' || sample.status === undefined}
	...
	<div class="flex gap-8">
		...
		<div class="flex-[3] max-h-[808px] overflow-y-auto">
