...
import type { SchedulerYield } from '@sjsf/form/lib/scheduler';
import type { Schema, SchemaValue } from '@sjsf/form/core';
import {
	abortPrevious,
	debounce,
	useMutation,
...
import { setFromContext, type FormContext, type IconOrTranslationData } from './context/index.js';
...
export interface UseFormOptions<T, E> {
	...
	//
	initialValue?: T;
	initialErrors?: Errors<E> | Map<string, FieldErrors<E>> | ValidationError<E>[];
	/**
	 ...
	 * - validation timeout
	 */
	onValidationFailure?: (state: FailedMutation<unknown>, e: SubmitEvent) => void;
	/**
...
export type ValidationProcessErrorTranslation = (state: FailedMutation<unknown>) => string;
 ...
 */
export function createForm<T, E>(options: UseFormOptions<T, E>): [FormContext, FormAPI<T, E>] {
...
>(options: O): FormApiAndContext<T, E> {
	...
	let value = $state(
		merger.mergeFormDataAndSchemaDefaults(options.initialValue as Value, options.schema)
	);
	...
	const pseudoIdSeparator = $derived(options.pseudoIdSeparator ?? DEFAULT_PSEUDO_ID_SEPARATOR);
	...
	const schedulerYield: SchedulerYield = ({ signal }: Parameters<SchedulerYield>[0]) =>
	...
	function validateSnapshot(snapshot: SchemaValue | undefined, signal: AbortSignal) {
		const errors = options.validator.validateFormData(validationSchema, snapshot, signal);
	...
	const fieldsValidation = useMutation({
		...
		get combinator() {
			return (
				...
				debounce(
					abortPrevious,
					options.fieldsValidationDebounceMs ?? DEFAULT_FIELDS_VALIDATION_DEBOUNCE_MS
				)
			);
		},
		get delayedMs() {
	...
	function reset() {
		...
		isChanged = false;
		errors.clear();
		value = merger.mergeFormDataAndSchemaDefaults(options.initialValue as Value, options.schema);
	}
