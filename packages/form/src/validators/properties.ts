...
import type { Schema, SchemaDefinition } from '@/core/index.js';
...
export function createVirtualAdditionalPropertiesRemover(cache = new WeakMap<Config, Config>()) {
	...
	return {
		validateAdditionalPropertyKey(key, { patternProperties, additionalProperties }) {
			...
			return patternProperties === undefined ||
				...
				? []
				: [typeof error === 'string' ? error : error({ key, patternProperties })];
		}
	};
}
