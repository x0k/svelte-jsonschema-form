...
describe("parseSchemaValue", async () => {
      ...
      X21mX21mkeyX219input: {
        [DEFAULT_ID_PREFIX]: [
          undefined,
          {
            ...
            AdditionalX1wpropertyX2191: {
              X21mX21m: "Additional property-1 changed",
            },
          },
        ],
      },
  ...
  it("Should parse schema with dependencies", async () => {
    ...
    await expect(
      parseSchemaValue(
        c.signal,
        opts({ schema, input, idPrefix: input[SJSF_ID_PREFIX] as string })
      )
    ).resolves.toEqual({
      conditional: {
