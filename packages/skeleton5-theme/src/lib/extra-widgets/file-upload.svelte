<script lang="ts" module>
  ...
  declare module "@sjsf/form" {
    interface ComponentProps {
      skeleton5FileUploadWidget: WidgetCommonProps<FileList> & {
        multiple: boolean;
        loading: boolean;
        processing: boolean;
      };
    ...
    interface ComponentBindings {
      skeleton5FileUploadWidget: "value";
    }
    interface UiOptions {
...
<script lang="ts">
  ...
  let {
    ...
    value = $bindable(),
    errors,
  }: ComponentProps["skeleton5FileUploadWidget"] = $props();
    ...
    untrack(() => {
      ...
      if (value === undefined) {
        ...
        const toAdd = Array.from(value).filter(
          (fl) =>
            a.acceptedFiles.find(
              (f) => fl.name === f.name && fl.size === f.size
            ) === undefined
        );
        if (toAdd.length > 0) {
          a.setFiles(toAdd);
...
<FileUpload.Provider
  class="w-full"
  value={api}
  {...customInputAttributes(ctx, config, "skeleton5FileUpload", {})}
>
  <FileUpload.Dropzone>
