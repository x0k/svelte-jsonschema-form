<script lang="ts" module>
  ...
  declare module "@sjsf/form" {
    interface ComponentProps {
      skeleton5SliderWidget: WidgetCommonProps<number>;
    }
    interface ComponentBindings {
      skeleton5SliderWidget: "value";
    }
    interface UiOptions {
...
<script lang="ts">
  ...
  let {
    ...
    handlers,
    errors,
  }: ComponentProps["skeleton5SliderWidget"] = $props();
...
<Slider
  ...
  {...customInputAttributes(ctx, config, "skeleton5Slider", {
    ids: {
      hiddenInput() {
        return id;
      },
