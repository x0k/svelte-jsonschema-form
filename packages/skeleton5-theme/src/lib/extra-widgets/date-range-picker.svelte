...
<script lang="ts">
  ...
  const inputIds = $derived(
    properties.map((p) => getId(ctx, getChildPath(ctx, config.path, p)))
  );
</script>
...
<DatePicker
  ...
  value={typeof value?.start === "string"
    ...
    : undefined}
  {...customInputAttributes(ctx, config, "skeleton5DateRangePicker", {
    selectionMode: "range",
    ids: {
      input: (i) => inputIds[i],
    },
...
>
  ...
  <Portal {...uiOptionProps("skeleton5DateRangePickerPortal")({}, config, ctx)}>
